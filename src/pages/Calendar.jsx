  ...
  const renderHeader = () => (
    <div className="flex items-center justify-between px-2 mb-8">
      <div>
        ...
        <p className="text-xs text-text-muted font-bold uppercase tracking-widest mt-1">Cloud-Synced Schedule 🛰️</p>
      </div>
      ...
      <div className="space-y-8">
        <div className="nv-card space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black text-text-primary uppercase tracking-tight">Active Plans</h2>
          ...
          <div className="space-y-4">
            <AnimatePresence mode="popLayout">
              {events.filter(e => e.event_date === format(selectedDate, 'yyyy-MM-dd')).map(e => (
                <motion.div key={e.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex items-center gap-4 group">
        ...
        {showAdd && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="nv-card w-full max-w-md p-10 space-y-8 relative z-10">
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">New Event</h2>
              <form onSubmit={handleAddEvent} className="space-y-6">
                <input name="title" required placeholder="Event Title" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-nuvio-purple-500" />
                ...
                <button type="submit" className="w-full nv-btn-primary h-14 uppercase tracking-widest text-xs">Save to Cloud</button>
              </form>
            </motion.div>
          </div>
        )}
